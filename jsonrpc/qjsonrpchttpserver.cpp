...
QJsonRpcHttpServerSocket::QJsonRpcHttpServerSocket(QObject *parent)
    : QTcpSocket(parent),m_requestParser(0)
{
...
qint64 QJsonRpcHttpServerSocket::writeData(const char *data, qint64 maxSize)
{
    ...
    if (document.isObject()) {
        ...
        qint64 bytesWritten = QTcpSocket::writeData(m_responseBuffer.constData(), m_responseBuffer.size());
        close();
...
void QJsonRpcHttpServerSocket::sendErrorResponse(int statusCode)
{
    ...
    QTcpSocket::writeData(responseHeader.constData(), responseHeader.size());
    close();
}
